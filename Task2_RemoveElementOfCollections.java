...
public class Task2_RemoveElementOfCollections {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) < 4){
                list.remove(i);
                //i--; // sometimes this is work but not all time
        }
        //chat gbt solution fault
...
//            }
        ...
        list2.removeIf( p -> p < 4);// implicitly iterable

        System.out.println("list2 = " + list2);

        System.out.println("***********************************************************************");
        ArrayList<String> names = new ArrayList<>();
        names.addAll(Arrays.asList("Java","Python","C#", "Ruby","Java"));

        names.removeIf(each -> each.startsWith("J")); // each -> , we can write
